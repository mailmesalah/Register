...
Begin VB.Form FRegister 
   ...
   Begin VB.CommandButton CExtendKey 
      Caption         =   "Get Extend Key"
      BeginProperty Font 
         Name            =   "Century Gothic"
         Size            =   12
         Charset         =   0
   ...
   Begin VB.TextBox TSerial 
      ...
      Width           =   7215
   End
   Begin VB.CommandButton CGetKey 
      Caption         =   "Get Key"
      BeginProperty Font 
         Name            =   "Century Gothic"
         Size            =   12
...
Dim rs As Recordset, sKey As String, sMac As String
...
Private Sub CExtendKey_Click()
    TKey.Text = GetDateExtendKey(Trim(TSerial.Text))
End Sub

Private Sub CGetKey_Click()
    Dim sMacAd As String, r As Long, pubKey As String
    
    sMacAd = Decrement(Decrement(Mid(Trim(TSerial.Text), 1, 1)))
    r = 2
    
    While r <= Len(Trim(TSerial.Text))
        If ((r Mod 2) = 1) Then
            sMacAd = sMacAd & "-"
        End If
        
        sMacAd = sMacAd & Decrement(Decrement(Mid(Trim(TSerial.Text), r, 1)))
        r = r + 1
    Wend
    
    sMacAd = Left(Base64EncodeString(sMacAd), 10)
    pubKey = Left(Base64EncodeString("EC:A8:6B:FC:7E:A7"), 10)
    Dim sKey As String
    r = 1
    While r <= 10
        If ((r Mod 2) = 0) Then
            sKey = sKey & Mid(sMacAd, r, 1)
        Else
